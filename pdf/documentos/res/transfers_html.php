...
<page backtop="15mm" backbottom="15mm" backleft="15mm" backright="15mm" style="font-size: 12pt; font-family: arial" >
    ...
    <table cellspacing="0" style="width: 100%;">
		...
		<?php } //end foreach ?>   
            <td style="width: 75%;text-align:right">
                <h2 style="color: #5c66bf;"> Reporte de Transferencias</h2>
            </td>
        </tr>
    </table>
    <br>
    ...
    <table cellspacing="0" style="width: 100%; border: solid 1px #5c66bf; background: #5c66bf; color:white;text-align: center; font-size: 10pt;padding:1mm;">
        <tr>
            ...
            <th style="width: 15%">CTA. DEBITAR</th>
            <th style="width: 20%">CTA. ACREDITAR</th>
    ...
    <?php
		...
		$sTable="accounts_transfers";
			...
			while ($key=mysqli_fetch_array($query)) {
				$id_from=$key['account_from_id'];
				$id_to=$key['account_to_id'];
				$description =$key['note'];


				$sql_acc=mysqli_query($con,"select name from accounts where id='$id_from'");
				$rw=mysqli_fetch_array($sql_acc);
				$acc_name=$rw['name'];

				$sql_acc2=mysqli_query($con,"select name from accounts where id='$id_to'");
				$r=mysqli_fetch_array($sql_acc2);
				$acc_name_to=$r['name'];
	?>
			<tr>
				<td style="width: 10%; text-align: left"><?php echo date("d/m/Y", strtotime($key['created_at'])); ?></td>
				<td style="width: 15%; text-align: left;"><?php echo $acc_name ?></td>
				<td style="width: 20%; text-align: center"><?php echo $acc_name_to; ?></td>
				<td style="width: 40%; text-align: center"><?php echo $description; ?></td>
       ...
       ?>
		...
		<!--  <tr>
			<td style='text-align:right; border-top:solid 1px #5c66bf' colspan=4><strong>TOTAL <?php echo $coin_c;?></strong> </td>
