...
else
{
			...
			mysqli_query($con, "DROP TABLE IF EXISTS user, income, expenses, configuration, category_income, category_expence");
			...
			$user_password_hash = sha1(md5(mysqli_real_escape_string($con,(strip_tags($password,ENT_QUOTES)))));
			...
			mysqli_query($con, "insert into user (id, status,is_deleted, name,  password, email, profile_pic, is_admin, created_at)
			values(NULL, '1','0','Sistemas Web','$user_password_hash','$username','default.png','1','$date_added')");
  ...
  <div style="float: left; width: 569px;">
    ...
    <form  method="post"  id="form" name='form' onsubmit="return validar();">
      ...
      <div style="background: #F7F7F7; border: 1px solid #DDDDDD; padding: 10px; margin-bottom: 15px;">
        <table>
          <tr>
            <td width="185"><span class="required">*</span>E-mail:</td>
            <td><input type="email" name="username" id="username" value="admin@admin.com" required/>
			</td>
          </tr>
          <tr>
